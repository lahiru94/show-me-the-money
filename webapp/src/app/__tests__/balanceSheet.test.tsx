...
describe('Properly Rendered Balance Sheet', () => {
  ...
  test('calls fetch with correct URL', async () => {
    render(await BalanceSheet({}));
    expect(fetch).toHaveBeenCalledWith(`${process.env.BACKEND_URL}/api/balance-sheet`);
    expect(fetch).toHaveBeenCalledTimes(1);
  });
});

describe('Error Handling in Balance Sheet', () => {
  ...
  test('renders error message when fetch fails', async () => {
    render(await BalanceSheet({}));
    expect(screen.getByText("Couldn't load balance sheet")).toBeDefined();
  });
});
